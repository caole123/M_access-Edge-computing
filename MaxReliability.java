...
public class MaxReliability {
    public static double exe(Application G,List<Task> givenTaskList,List<Processor> givenProcessorList, Integer[][] compMartrix, Integer[][] commMartrix, Double rgoal, Double deadline, boolean print ) throws IOException, ClassNotFoundException {
        ...
        for (int i = 1; i< givenTaskList0.size();i++ ){
            ...
            if(v_i.getRunCpuNumber()/G.getLocalCpuNumber()>v_i.getreqT()) {
                ...
                System.out.println("分支1后在本地执行的任务:"+v_i);
            }
        }
        //分支2
        int J;
        ...
        Arrays.sort(TaskRunCpuNumber,1,givenTaskList0.size()+1);//从大到小排序
        Arrays.sort(TaskData,1,givenTaskList0.size()+1);
        for (int i = 1;i<TaskRunCpuNumber.length+1;i++){
        ...
        for (int i = 1; i< givenTaskList0.size();i++){
            ...
            for (int j = 1;j<givenTaskList0.size();j++){
                for (int k = 1; k<givenTaskList0.size();k++){

                    if (Ver_i.getData()==TaskData[j]&&Ver_i.getRunCpuNumber()!=TaskRunCpuNumber[k]){
                        Ver_i.setOpen(false);
                    }
                    if (Ver_i.getData()==TaskData[j]&&Ver_i.getRunCpuNumber()==TaskRunCpuNumber[k])    {
                        for (int h = 1;h<givenProcessorList0.size();h++)
                        {         Processor p_h = givenProcessorList0.get(h);
                            if (G.getEnergypercent()*(Ver_i.getRunCpuNumber()/G.getLocalCpuNumber())>G.getTrPower()*(Ver_i.getData()/G.getUpdataRate())){
                                Ver_i.setOpen(false);
                            }
                        }
                    }
